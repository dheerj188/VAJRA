# SOLVES LINEAR EQUATIONS ACROSS THE NODES 
...
for i in range(70):  # Repeat the tasks 70 times for demonstration
    for script_name in task_scripts.keys():
        TASK_QUEUE.put(script_name)
...
def execute_task_on_node(node, task_file):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        sftp.put(task_file, f'/tmp/{task_file}')
        sftp.close()
