#SCHEDULES A 2 X 2 MATRIX ACROSS EACH NODE AND EACH NODE CALCULTES IT 70 TIMES
...
LATENCIES = {node['host']: 0 for node in NODES}
...
latencies = [LATENCIES[node] for node in nodes]

fig, ax1 = plt.subplots()

color = 'tab:blue'
ax1.set_xlabel('Nodes')
ax1.set_ylabel('Average Task Time (seconds)', color=color)
ax1.bar(nodes, averages, color=color, alpha=0.6, label='Task Time')
ax1.tick_params(axis='y', labelcolor=color)

ax2 = ax1.twinx()  
color = 'tab:red'
ax2.set_ylabel('Network Latency (ms)', color=color)
ax2.plot(nodes, latencies, color=color, marker='o', label='Latency')
ax2.tick_params(axis='y', labelcolor=color)

fig.tight_layout()
plt.title('Average Task Time and Network Latency per Node')
plt.show()
